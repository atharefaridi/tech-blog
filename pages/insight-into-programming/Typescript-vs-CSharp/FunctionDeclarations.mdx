...
| **Function Overloading** | No true overloading; uses union types or signature declarations. Example: `function fn(a: string | number)`. | Supports true overloading with different parameter types/counts. Example: `void Fn(string s)`, `void Fn(int i)`. |
| **Return Types** | Optional with type inference; supports `void`, `any`, union types. Example: `function fn(): string | number`. | Mandatory explicit return types; `void` for no return. Example: `string Fn()`. |
